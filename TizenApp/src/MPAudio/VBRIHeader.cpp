...
VBRIHeader* VBRIHeader::FindHeader(const MPAFrame* pFrame)
{
	// VBRI header always at fixed offset
	int dwOffset = pFrame->m_dwOffset + MPA_HEADER_SIZE + 32;
	
	// VBRI ID found?
	if (!CheckID(pFrame->m_pStream, dwOffset, 'V', 'B', 'R', 'I'))
		return NULL;
	
	return new VBRIHeader(pFrame, dwOffset);
}


VBRIHeader::VBRIHeader(const MPAFrame* pFrame, int dwOffset) :
	VBRHeader(pFrame->m_pStream, dwOffset)
{
	...
	2		size of table entry (max. size = 4 unsigned char (must be stored in an integer))
...
VBRIHeader::~VBRIHeader(void)
{
}

int VBRIHeader::SeekPosition(float& fPercent) const
{
	return SeekPositionByTime((fPercent/100.0f) * m_dwLengthSec * 1000.0f);
}

int VBRIHeader::SeekPositionByTime(float fEntryTimeMS) const
{
	 int i=0,  fraction = 0;
	int dwSeekPoint = 0;
	...
	dwSeekPoint -= (int)((float)m_pnToc[i-1] * (float)(fraction) 
