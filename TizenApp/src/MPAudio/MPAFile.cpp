#include "MPAudio/MPAException.h"
#include "MPAudio/MPAFile.h"


/// CMPAFile
//////////////////////////////////////////



MPAFile::MPAFile( char* szFile)
{
	int dwOffset = 0;
	m_pStream = new MPAFileStream(szFile);
	m_pTags = new Tags(m_pStream);
	
	// find first valid MPEG frame
	m_pFirstFrame = new MPAFrame(m_pStream, dwOffset, true, false, false, NULL);

	// check for VBR header
	m_pVBRHeader = m_pFirstFrame->FindVBRHeader();
	
	CalcBytesPerSec();
	// CalcTotalFrameCount();
}

// destructor
MPAFile::~MPAFile(void)
{
	if (m_pVBRHeader){
		delete m_pVBRHeader;
		m_pVBRHeader = 0;
	}

	if( m_pFirstFrame ){
		delete m_pFirstFrame;
		m_pFirstFrame = 0;
	}

	if( m_pTags) {
		delete m_pTags;
		m_pTags = 0;
	}

	if( m_pStream ){
		delete m_pStream;
		m_pStream;
	}
}
#define MAX_EMPTY_FRAMES 25

void MPAFile::CalcTotalFrameCount(){
	int count = 0;
	MPAFrame* checkFrame = GetFrame( First, NULL );

	if( checkFrame ){
		int samplePerFrame = checkFrame->m_pHeader->m_dwSamplesPerFrame;
		int sampleRate = checkFrame->m_pHeader->m_dwSamplesPerSec;

		m_iEachFrameMillisecond = (samplePerFrame / sampleRate) * 1000;
	}
	

	while( checkFrame ){
		count++;
		checkFrame = GetFrame( Next, checkFrame );
	}

	m_iTotalFrameCount = count;
}

// try to guess the bitrate of the whole file as good as possible
void MPAFile::CalcBytesPerSec()
{
	// in case of a VBR header we know the bitrate (if at least the number of frames is known)
	if (m_pVBRHeader && m_pVBRHeader->m_dwFrames)
	{
		int dwBytes = m_pVBRHeader->m_dwBytes;
		// number of bytes can be guessed
		if (!dwBytes)
			m_pVBRHeader->m_dwBytes = GetFileSize();
		
		m_dwBytesPerSec = m_pFirstFrame->m_pHeader->GetBytesPerSecond(m_pVBRHeader->m_dwFrames, dwBytes);
		return;
	}
	
	// otherwise we have to guess it
	
	// go through all frames that have a lower bitrate than 48kbit
	MPAFrame* pFrame = m_pFirstFrame;
	int dwFrames = 0;
	bool bDeleteFrame = false;
	
	while (pFrame && pFrame->m_pHeader->m_dwBitrate <= 48000)
	{
		pFrame = GetFrame(Next, pFrame, bDeleteFrame);
		if (dwFrames++ > MAX_EMPTY_FRAMES)	
			break;
		bDeleteFrame = true;
	};

	if (!pFrame) {
		pFrame = m_pFirstFrame;
		bDeleteFrame = false;
	}
	m_dwBytesPerSec = pFrame->m_pHeader->GetBytesPerSecond();
	
	if (bDeleteFrame)
		delete pFrame;
	//return true;
}

#define MIN_FRAME_SIZE 24	// MPEG2, LayerIII, 8kbps, 24kHz => Framesize = 24 Bytes

MPAFrame* MPAFile::GetFrame(MPAFile::GetType Type, MPAFrame* pFrame, bool bDeleteOldFrame, int dwOffset)
{
	MPAFrame* pNewMPAFrame;
	MPAHeader* pCompHeader = NULL;
	bool bSubsequentFrame = true;
	bool bReverse = false;
	bool bExactOffset = true;
	
	switch(Type)
	{
		case First:
			dwOffset = GetBegin();
			bSubsequentFrame = true;
			bExactOffset = false;
			break;

		case Last:
			if( m_pFirstFrame ){
				dwOffset = GetEnd() - m_pFirstFrame->m_dwFrameSize;
			}
			else{
				dwOffset = GetEnd() - MIN_FRAME_SIZE;
			}
			
			bReverse = true;
			bExactOffset = false;
			pCompHeader = m_pFirstFrame->m_pHeader;
			break;

		case Next:
			if (!pFrame)
				return NULL;

			pCompHeader = m_pFirstFrame->m_pHeader;
			dwOffset = pFrame->GetSubsequentHeaderOffset();
			break;

		case Prev:
			if (!pFrame)
				return NULL;

			if( pFrame ){
				dwOffset = pFrame->m_dwOffset - pFrame->m_dwFrameSize - SAFE_PADDING_BIT_FOR_PREV_BIT;
			}
			else{
				dwOffset = pFrame->m_dwOffset - MIN_FRAME_SIZE;
			}

			bReverse = false;
			bExactOffset = false;
			pCompHeader = NULL;
			break;

		case Resync:
			bSubsequentFrame = true;
			bExactOffset = false;
			//pCompHeader = m_pFrame->m_pHeader;
			break;

		default:
			return NULL;
	}

	try {
		pNewMPAFrame = new MPAFrame(m_pStream, dwOffset, bSubsequentFrame, bExactOffset, bReverse, pCompHeader); 
	}

	catch(MPAException& e) {

		// try a complete resync from position dwOffset
		if (Type == Next) {
			return GetFrame(Resync, pFrame, bDeleteOldFrame, dwOffset);
		}
		pNewMPAFrame = NULL;
	}

	if (pFrame && bDeleteOldFrame)
		delete pFrame;

	return pNewMPAFrame;
}

int MPAFile::getPlayTimeSec( ) {
	return m_iEachFrameMillisecond * m_iTotalFrameCount;
}

int MPAFile::getTotalFrameCount(){
	return m_iTotalFrameCount;
}
